...
export default function NewAppointmentPage() {
  ...
  useEffect(() => {
    ...
    if (!session) {
      ...
      if (staffError) {
        console.error("Personeller alınamadı:", staffError.message);
      } else {
        setStaffList(staffData || []);
        if (staffData && staffData.length > 0) {
          setStaffName(staffData[0].full_name);
        }
      }

      if (serviceError) {
        console.error("Hizmetler alınamadı:", serviceError.message);
      } else {
...
if (checkError) {
  setLoading(false);
  setMessage("Randevu kontrol edilirken hata oluştu: " + checkError.message);
  return;
}
...
if (existingAppointment) {
  setLoading(false);
  setMessage("Bu personel için bu saatte zaten randevu var.");
  return;
}
...
const { error } = await supabase.from("appointments").insert([
  {
    ...
    if (error) {
      setMessage("Randevu kaydedilemedi: " + error.message);
      return;
    }
  ...
  if (pageLoading) {
    return <p className="text-slate-500">Form yükleniyor...</p>;
  }
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Yeni Randevu</h1>
          <p className="text-slate-500 mt-2">
            Yeni müşteri randevusu oluşturun
          </p>
        </div>
